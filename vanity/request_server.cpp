...
void RequestServer::dispatch_op(Client &client, operation_t op, Request& request, bool end) {
	...
	switch (op) {
		...
		{
			request_ping(client, extract_exact<STR>(request, end));
			break;
		}
		...
		case operation_t::PIPE:
		{
			...
			ensure_end_if(request, end);
			break;
		}

		case operation_t::AUTH:
		{
			auto [username, password] = extract_exact<STR, STR>(request, end);
		...
		{
			request_del_user(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::CHANGE_PASSWORD:
		{
			request_change_password(client, extract_exact<STR>(request, end));
			break;
		}

		case operation_t::SWITCH_DB:
		{
			request_switch_db(client, extract_exact<INT>(request, end));
			break;
		}
		...
		{
			request_del(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::TYPE:
		{
			request_type(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::EXISTS:
		{
			request_exists(client, extract_exact<STR>(request, end));
			break;
		}
		...
		{
			auto [from, to] = extract_exact<STR, STR>(request, end);
		...
		{
			auto [from, to] = extract_exact<STR, INT>(request, end);
		...
		{
			auto [key, expiry] = extract_exact<STR, FLOAT>(request, end);
			request_set_expiry(client, key, expiry);
			break;
		}
		case operation_t::GET_EXPIRY:
		{
			request_get_expiry(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::CLEAR_EXPIRY:
		{
			request_clear_expiry(client, extract_exact<STR>(request, end));
			break;
		}

		case operation_t::GET:
		{
			request_get(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::STR_SET:
		{
			auto [key, value] = extract_exact<STR, STR>(request, end);
			request_str_set(client, key, value);
			break;
		}
		case operation_t::INT_SET:
		{
			auto [key, value] = extract_exact<STR, INT>(request, end);
			request_int_set(client, key, value);
			break;
		}
		case operation_t::FLOAT_SET:
		{
			auto [key, value] = extract_exact<STR, FLOAT>(request, end);
		...
		{
			request_str_len(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::MANY_GET:
		{
			request_many_get(client, extract_exact<ARR>(request, end));
			break;
		}

		case operation_t::LIST_LEN:
		{
			request_list_len(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::LIST_GET:
		{
			auto [key, index] = extract_exact<STR, INT>(request, end);
			request_list_get(client, key, index);
			break;
		}
		case operation_t::LIST_SET:
		{
			auto [key, index, value] = extract_exact<STR, INT, STR>(request, end);
			request_list_set(client, key, index, value);
			break;
		}
		case operation_t::LIST_PUSH_LEFT:
		{
			auto [key, value] = extract_exact<STR, LIST>(request, end);
		...
		{
			auto [key, count] = extract_exact<STR, INT>(request, end);
		...
		{
			auto [key, start, stop] = extract_exact<STR, INT, INT>(request, end);
		...
		{
			auto [key, value, count] = extract_exact<STR, STR, INT>(request, end);
		...
		{
			auto [key, values] = extract_exact<STR, SET>(request, end);
			request_set_add(client, key, std::move(values));
			break;
		}
		case operation_t::SET_ALL:
		{
			request_set_all(client, extract_exact<STR>(request, end));
			break;
		}
		...
		{
			request_set_len(client, extract_exact<STR>(request, end));
			break;
		}
		...
		{
			auto [source, dest, value] = extract_exact<STR, STR, STR>(request, end);
			request_set_move(client, source, dest, value);
			break;
		}
		case operation_t::SET_UNION:
		{
			request_set_union(client, extract_exact<ARR>(request, end));
			break;
		}
		case operation_t::SET_UNION_INTO:
		{
			auto [dest, keys] = extract_exact<STR, ARR>(request, end);
			request_set_union_into(client, dest, keys);
			break;
		}
		case operation_t::SET_UNION_LEN:
		{
			request_set_union_len(client, extract_exact<ARR>(request, end));
			break;
		}
		case operation_t::SET_INTERSECT:
		{
			request_set_intersection(client, extract_exact<ARR>(request, end));
			break;
		}
		...
		{
			request_set_intersection_len(client, extract_exact<ARR>(request, end));
			break;
		}
		case operation_t::SET_DIFF:
		{
			auto [key1, key2] = extract_exact<STR, STR>(request, end);
			request_set_difference(client, key1, key2);
			break;
		}
		case operation_t::SET_DIFF_INTO:
		{
			auto [dest, key1, key2] = extract_exact<STR, STR, STR>(request, end);
		...
		{
			auto [key, hash] = extract_exact<STR, HASH>(request, end);
			request_hash_set(client, key, std::move(hash));
			break;
		}
		case operation_t::HASH_ALL:
		{
			request_hash_all(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::HASH_GET:
		{
			auto [key, hash_key] = extract_exact<STR, STR>(request, end);
		...
		{
			request_hash_len(client, extract_exact<STR>(request, end));
			break;
		}
		...
		{
			auto [key, hash_key] = extract_exact<STR, ARR>(request, end);
			request_hash_remove(client, key, hash_key);
			break;
		}
		case operation_t::HASH_KEYS:
		{
			request_hash_keys(client, extract_exact<STR>(request, end));
			break;
		}
		case operation_t::HASH_VALUES:
		{
			request_hash_values(client, extract_exact<STR>(request, end));
			break;
		}
		...
		{
			auto [key, hash_keys] = extract_exact<STR, ARR>(request, end);
...
void RequestServer::dry_dispatch_op(operation_t op, Request& request, bool end) {
	...
	switch (op) {
		...
		case operation_t::AUTH:
		{
			extract_exact<STR, STR>(request, end);
			break;
		}
		case operation_t::ADD_USER:
		{
		...
		case operation_t::COPY_TO_DB:
		{
			extract_exact<STR, INT>(request, end);
			break;
		}
		case operation_t::MOVE_TO_DB:
		{
		...
		case operation_t::SET_EXPIRY:
		{
			extract_exact<STR, FLOAT>(request, end);
			break;
		}
		case operation_t::GET_EXPIRY:
		{
		...
		case operation_t::LIST_SET:
		{
			extract_exact<STR, INT, STR>(request, end);
			break;
		}
		case operation_t::LIST_PUSH_LEFT:
		{
			extract_exact<STR, LIST>(request, end);
			break;
		}
		case operation_t::LIST_PUSH_RIGHT:
		{
		...
		case operation_t::LIST_RANGE:
		{
			extract_exact<STR, INT, INT>(request, end);
			break;
		}
		case operation_t::LIST_TRIM:
		{
		...
		case operation_t::LIST_REMOVE:
		{
			extract_exact<STR, STR, INT>(request, end);
			break;
		}

		case operation_t::SET_ADD:
		{
			extract_exact<STR, SET>(request, end);
			break;
		}
		case operation_t::SET_ALL:
		{
		...
		case operation_t::SET_MOVE:
		{
			extract_exact<STR, STR, STR>(request, end);
			break;
		}
		case operation_t::SET_UNION:
		{
		...
		case operation_t::SET_UNION_INTO:
		{
			extract_exact<STR, ARR>(request, end);
			break;
		}
		case operation_t::SET_UNION_LEN:
		{
		...
		case operation_t::HASH_SET:
		{
			extract_exact<STR, HASH>(request, end);
			break;
		}
		case operation_t::HASH_ALL:
		{
