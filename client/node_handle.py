...
class NodeHandle:
    ...
    def new_cluster(self, cluster_key: str):
        """
        Create a new cluster on the node.
        """
        with self.own_client() as client:
            response = client.cluster_new(cluster_key)
            if not response.is_ok():
                raise RuntimeError(f"Failed to create new cluster: {response.value}")

    def join_cluster(self, cluster_key: str, host: str, port: int):
        """
        Join a cluster on the node.
        """
        with self.own_client() as client:
            response = client.cluster_join(cluster_key, host, port)
            if not response.is_ok():
